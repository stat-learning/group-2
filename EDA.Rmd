...
# Uploading a section of the Yelp dataset: 
```{r}
library(jsonlite)
business <- stream_in(file("business.json"))
```

# We want to restrict the data frame to include only restauraunts. We're trying to do this by including only rows that mention the category "restauraunts" in the categories column. 
```{r message = FALSE}
install.packages("rlist")
library(rlist)
library(dplyr)
library(tidyverse)
clean.business <- na.omit(business)
